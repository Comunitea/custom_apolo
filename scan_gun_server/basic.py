...
class LineReceiver(protocol.Protocol, _PauseableMixin):
    ...
    def check_function_keys(self, data):
        ...
        if data == KEY_F1:
            res = "F1" + self.delimiter
        elif data == KEY_F2:
            res = "F2" + self.delimiter
        elif data == KEY_F3:
            res = "F3" + self.delimiter
        elif data == KEY_F4:
            ...
            res = "F7" + self.delimiter
        elif data == KEY_F8:
            res = "F8" + self.delimiter
        elif data == KEY_F9:
            res = "F9" + self.delimiter
        elif data == KEY_F10:
            res = "F10" + self.delimiter
        elif data == KEY_F11:
            res = "F11" + self.delimiter
        elif data == KEY_F12:
            res = "F12" + self.delimiter
        elif data == KEY_UP:
            res = "KU" + self.delimiter
        elif data == KEY_DOWN:
            res = "KD" + self.delimiter
        else:
            res = data
        return res

    def dataReceived(self, data_):
        """
        Protocol.dataReceived.
        Translates bytes into lines, and calls lineReceived (or
        rawDataReceived, depending on mode.)
        """
        data = self.check_function_keys(data_)
