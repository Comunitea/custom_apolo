...
class ScanGunProtocol(LineReceiver):
    ...
    def _snd(self, line='\n', message='', custom_format=True):
        ...
        self.last_send = line or ' ' + message or ' '
    ...
    def handle_list_ubi_ops(self, line, confirm=False):
        ...
        if self.ops and not order_line:
            ...
            if line in self.ops.keys():
                self.handle_list_ubi_ops(u"%s%s"%(PRE_PACK,self.ops[line]['pack_id']))
                return
            if line =='0':
    ...
    def print_packs(self):
        ...
        for pack in self.packs:
            packs.append(pack['id'])
        res = self.factory.odoo_con.print_packs_from_gun(self.user_id, packs)
        return res
