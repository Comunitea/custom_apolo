...
class ScanGunProtocol(LineReceiver):
    ...
    def reset_self_task(self):
        self.camera_id = False # Camara activa, por la que se busca
    ...
    def _snd_error(self, str, message):
        self.next_str = str
        message = u'\n' * 25 + message
    ...
    def _snd(self, line='\n', message=''):
        ...
        if self.debug==True:
            clean +='Menu: ' + str(self.last) + \
                    ...
                    ' \nOp id: ' + str(self.op_id)+ u'\n' * 5
        self.last = '-'
        if self.user_id:
            cabecera = self.user_name + ' (' + self.state + ',' + str(self.step) + ')'
        else:
            cabecera=''
        delimiter = "\n*******************************\n"
        if message !='':
        ...
        if not line:
            line = "\n"
        snd_str = (clean +cabecera + delimiter + line + message).encode("UTF-8")
    ...
    def get_str_form_repo_ops(self):
        ...
        if self.step >8:
            keys += "<" + KEY_CONFIRM +"> Ok OP"
        keys += "<" + KEY_VOLVER + "> Atras"
        if self.show_keys:
            strg += delimiter + keys
        return strg

    def handle_form_repo_ops(self, line):
        ...
        if not order_line:
            ...
            #NOS MOVEMOS POR LOS FORMULARIOS DE OPERACIONES
            if line == KEY_NEXT:
                if self.num_order_list_ops < len(self.ops):
                    self.num_order_list_ops += 1
                self.step=0
                self._snd(self.get_str_form_repo_ops())
                return
            ...
            if line==KEY_CONFIRM and self.step ==2:
                self.step =9
                self._snd(self.get_str_form_repo_ops(), '')
                return#la cantidad introducida en unidades de stock


        if order_line == PRE_PACK and self.step <=2:
            #Es un paquete
            if op_['PAQUETE'] == line or op_['paquete_id'] == self.int_(line):
                #es un paquete de la lista de paquetes de esta tarea
                self.state = 'form_repo_ops'
                self.step = 1
                self._snd(self.get_str_form_repo_ops(), '')
                ...
                # aqui debemos comprobart si pertenece a otra operación
                #busy_pack, task, usuario  = chequear_paquet>)
                busy_pack, task, usuario = True, "tarea", "operario"
                if busy_pack:
                    message = u'\nPaquete no Válido.\n Tarea %s del usuario %s '% (task, usuario)
                    self._snd_error(self.get_str_form_repo_ops(), message)

        if order_line == PRE_LOT and self.step>=1:
            if op_['lote'] == line or op_['lot_id'] == self.int_(line):
                #es un paquete de la lista de paquetes de esta tarea
                #lot_in_package = comprobamod que este lote está en el packet escaneado
                lot_in_package = True
                if lot_in_package:
                    self.state = 'form_repo_ops'
                    self.step = 4
                    ...
                    message = u'\nNo hay ese lote en el paquete\nCambia el lote'
                    self._snd_error(self.get_str_form_repo_ops(), message)

        if self.step>1:
            line = self.float_(line)
            if line:
                self.new_qty=line
                self.step = 5
                self._snd(self.get_str_form_wave(), '')#la cantidad no es correcta,
                return

        if self.step == 5:
            if line == KEY_CONFIRM or line == '0':
                #import ipdb; ipdb.set_trace()
                #confirmamos en unidad de stock
                fc = self.factory.odoo_con.conv_units_from_gun(wave_['EAN'], wave_['uom_id'], wave_['uos_id'], 0)
                if fc == False:
                    fc = wave_['CANTIDAD']/wave_['uos_qty']
                wave_['uos_qty'] = self.new_qty * fc
                wave_['CANTIDAD'] = self.new_qty
                self.step=3
                self.waves[str(self.active_wave)]['uos_qty']=wave_['uos_qty']
                self.waves[str(self.active_wave)]['CANTIDAD']=wave_['CANTIDAD']
                self._snd(self.get_str_form_wave(), '')#la cantidad no es correcta,
                return

            if line == KEY_QTY:
                #import ipdb; ipdb.set_trace()
                #confirmamos en unidad de venta
                fc = self.factory.odoo_con.conv_units_from_gun(wave_['EAN'], wave_['uos_id'], wave_['uom_id'], 0)
                if fc == False:
                    fc = wave_['CANTIDAD']/wave_['uos_qty']
                wave_['uos_qty'] = self.new_qty
                wave_['CANTIDAD'] = self.new_qty* fc
                self.step=3
                self.waves[str(self.active_wave)]['uos_qty']=wave_['uos_qty']
                self.waves[str(self.active_wave)]['CANTIDAD']=wave_['CANTIDAD']
                self._snd(self.get_str_form_wave(), '')#la cantidad no es correcta,
                ...
                if line == KEY_CANCEL:
                    # modificamos la cantidad, el uos o lo que sea
                    # y marcamos todaas las operaciones como a revisar.
                    ok = True
                    if ok:
                        self.step=2
                        self.new_qty=0.00
            ...
            if self.step==9 and line==KEY_CONFIRM:
                import ipdb; ipdb.set_trace()
                #tenemos que marcar todas las operaciones como to_process = False, para
                #para laswave _ id
                #set_wave_ops_values(self, wave_id, user_id, field, value)
    ...
    def get_str(self, data_, header =''):
        ...
        if self.show_keys:
            strg += delimiter + keys
        return strg.decode('unicode-escape')

    def get_str_list_ops_anterior(self):
        self.last = "get_str_list_ops_anterior"
        self.ops = self.factory.odoo_con.get_ops(self.task_id, 'ubication')
        op_=self.ops[str(self.active_op)]
        not_vis = 0
        not_proc = 0
        for op in self.ops:
        ...
        strg = header #+ delimiter
        #lo que mostramos como a continuación de paquete
        #depende de si es ubicaion o picking o reposicion
        ...
        for k in range(self.num_order_list_ops, self.num_order_list_ops +MAX_NUM):
            if k <= len(self.ops):
                k_ = str(k)
                op = k_ + '>'
                if not self.ops[k_]['PROCESADO']:
                    op = self.inverse(op)
                op +=self.ops[k_]['PAQUETE'] + ': ' + self.ops[k_][after_PAQUETE] + '\n'
                strg += op
    ...
    def handle_form_wave(self, line, confirm=False):
        ...
        if not order_line:
            ...
            #     self._snd(self.get_str_form_wave(), '')
            #     return
            if line==KEY_CONFIRM and self.step ==3:
                self.step =9
                self._snd(self.get_str_form_wave(), '')
                return#la cantidad introducida en unidades de stock
            ...
            if self.step == 3:
                ...
                if float(wave_['CANTIDAD']) == line or\
                    float(wave_['uos_qty']) == line:
                    self.step = 9
                    self._snd(self.get_str_form_wave(), '')
                    return#la cantidad introducida en unidades de stock
                else:
        ...
        if order_line == PRE_PACK and self.step == 1:
           ...
           for op_ in self.waves:
                op = self.waves[op_]
                if op['PAQUETE'] == line or op['paquete_id'] == self.int_(line) and self.step == 1:
                    #es un paquete de la lista de paquetes de esta tarea
                    self.last_state = "list_waves"
                    self.active_op = op_
                    self.wave_id = op['ID']
    ...
    def get_str_form_wave(self):
        ...
        if self.step != 5 and self.step != 6:

            cantidad = str(wave_['CANTIDAD']) + ' ' + wave_['uom']  + '\n'
            cantidad_uos = str(wave_['uos_qty']) + ' ' + wave_['uos'] + '\n'
            menu_str += cantidad
            if self.step >= 5 :
                menu_str+=self.inverse(cantidad_uos)
            else:
                menu_str += cantidad_uos
            keys =''
            if self.step==9 or self.step ==3:
                keys += KEY_CONFIRM +' Confirmar'
            keys += KEY_CANCEL + ' Atras' + KEY_VOLVER + ' Volver'
            keys += KEY_WAVE_OPS + ' Operaciones'


        if self.step == 5 :
            #import ipdb; ipdb.set_trace()
            cantidad = "Cantidad: %s %s\n" %(str(self.new_qty),wave_['uom'])
            #cantidad += str(self.new_qty)  + ' ' + wave_['uom'] + '\n'
            menu_str += cantidad
            keys = KEY_CONFIRM+ ' Conf ' + \
                          KEY_QTY + ' para ' + wave_['uos'] + ' ' +  KEY_CANCEL + ' Cancel'

        if self.step == 6 :
            cantidad = "Nueva cantidad:\n"
            cantidad += str(self.new_qty)  + ' ' + wave_['uos'] + '\n'
            menu_str += cantidad
            keys = KEY_CONFIRM + ' para ' + wave_['uom'] + ' ' +  KEY_CANCEL + ' Cancel'

        menu_str = header + menu_str + keys
        return menu_str.decode('unicode-escape')
    ...
    def get_manual_menu(self):
        ...
        if self.vals['product_id']!=False:
            menu_str+=u"EAN:" + self.vals['product'] + u"\n"
        if self.vals['quantity']!=0:
            menu_str+=u"Cantidad:" + str(self.vals['quantity']) + u"\n"
        if self.vals['lot_id']!=False:
            menu_str+=u"Lote:" + self.vals['lot'] + u"\n"
        ...
        if self.step==0:
            menu_str+= self.inverse(u"Paquete o EAN\n")
        if self.step==1:
            menu_str+= self.inverse(u"EAN, origen o teclea cantidad\n")
            if self.vals['src_location_id']!= False:
                menu_str += "<" + KEY_CONFIRM + "> Seguir a destino\n"
        if self.step==2:
            menu_str+= self.inverse(u"Confirma cantidad\n(" + str(self.vals['quantity']) + u')\n')
        if self.step==3:
            menu_str+= self.inverse(u"Lote o scan origen\n")
            if self.vals['src_location_id']!= False:
                menu_str += "<" + KEY_CONFIRM + "> Seguir a destino\n"
        if self.step==4:
            menu_str+=self.inverse( u"scan origen\n")
        if self.step==5:
            menu_str+= self.inverse(u"scan destino\n")
        if self.step==6:
            menu_str+= u"1 -> Sin Paquete\n2 -> Palet\n3 -> Caja\n" + self.inverse("<" + KEY_CONFIRM + u"> Conf Movimiento (Sin Paquete)\n")
        if self.step==7:
            menu_str += "Confirmar:\n" + self.inverse("<" + KEY_YES+ u"> SI <" + KEY_CANCEL +u"> NO\n")
        ...
        res= menu_str.decode('unicode-escape')
    ...
    def handle_manual_menu(self, line):
        ...
        if self.step == 0:
            if order_line != False:
                if order_line == PRE_PACK:
                    package_id_ean = self.int_(line)
                    vals = self.factory.odoo_con.get_quant_pack_gun_info(self.user_id, package_id_ean, line)
                elif order_line == PRE_PROD:
                    vals = self.factory.odoo_con.get_product_gun_info(self.user_id,  str(line))
                else:
                    vals['exist'] = False
        ...
        if self.step==1:
            if line == KEY_CONFIRM and \
                            self.vals['src_location_id']!=False and\
                            self.vals['package_id']!= False:
                self.step = 5
                self._snd(self.get_manual_menu())
                return
            if order_line != False:
                ...
                if order_line ==PRE_LOC:
                    ...
                    vals = self.factory.odoo_con.get_product_gun_info(self.user_id, str(line_id))
                    if vals['exist'] != False:
                        #es un EAN
                        for k in vals:
                            self.vals[k]=vals[k]
                        ...
                        self._snd(self.get_manual_menu())
                        return
                elif order_line ==PRE_LOT:
                    vals = self.factory.odoo_con.get_lot_gun_info(self.user_id, line_id)
                    if vals['exist'] != False:
                        #es un lote
                        for k in vals:
                            self.vals[k]=vals[k]
            ...
            if order_line == False:
                ...
                if line in ['0','1','2','3']:
                    if line=='1':
                        ...
                        self.vals['do_pack'] ='palet'
                    elif line =="3":
                        self.vals['do_pack'] ='box'
    ...
    def get_views(self, line, message = ""):
        ...
        if self.views == 2: # filtrar pendientes
            if proc:
                #vamos a la siguiente O a la primera
                self.handle_ops_ubi(line=line)
            else:
        ...
        if line not in str_keys:
            ...
            try:
                ...
                if line=='0':
                    ...
                    self.camera_id = self.factory.menu_cameras[self.int_(line)][0]

                self.task_id = self.factory.odoo_con.get_task_of_type(self.user_id, self.camera_id, self.type, self.machine_id)
    ...
    def handle_machine_selected(self, line):
        ...
        if line not in str_keys:
            str_error = u"La opcion %s no es valida.\nReintentar\n" % line
            self._snd(str_error + self.get_machines_menu())
            return
        else:
            if line=='0':
                self.machine_id=False
        ...
        self.state = 'ubication'# self.type
    ...
    def menu_intro_qty (self, from_state, line):
        ...
        message = "Cantidad:/n <" + KEY_CONFIRM + "> para " + str(op_['CANTIDAD'])
    ...
    def handle_scan_location(self, line):
        ...
        if line == self.packet:
            ...
            message += "Nueva cantidad:(" + self.op_data['CANTIDAD'] + ")\n"
