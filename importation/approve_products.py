...
class import_products(object):
    ...
    def import_product(self):
        cont = 1
        product_ids = self.search('product.template', [('active','=',False)])
        all_lines = len(product_ids)
        print "products no: ", all_lines
        for product_id in product_ids:
            self.exec_workflow("product.template", "logic_validated", product_id)
            self.exec_workflow("product.template", "commercial_validated", product_id)
            self.exec_workflow("product.template", "active", product_id)
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        ...
        import_products(sys.argv[1], sys.argv[2], sys.argv[3])
